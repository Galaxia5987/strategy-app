...
import com.example.frc5987scoutingapp.data.model.qui
...
class AllianceViewModel(private val teamDao: teamDao) : ViewModel() {
    ...
    fun loadTeamData(teamId: Int, alliancePosition: AlliancePosition) {
        if (teamId == 5987) {
            val galaxiaSummary = quickGameStats(5987, generalNote = "קבוצתנו - Galaxia!", isGalaxia = true)
    ...
    private fun calculateSummary(teamId: Int, data: List<quickGameStats>): quickGameStats {
        if (data.isEmpty()) {
            return quickGameStats(teamId , generalNote = "אין נתונים זמינים")
        }


        val scores = data.map { stats ->
            val autoScore = quickGameStats.A_Pointes
